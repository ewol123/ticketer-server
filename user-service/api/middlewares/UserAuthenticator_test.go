...
func TestUserAuthenticator(t *testing.T){
	...
	r.Use(jwtauth.Verifier(tokenAuth), UserAuthenticator)
	...
	if status, resp := testRequest(t, ts, "GET", "/", nil, nil); status != 401 || resp != "Unauthorized\n" {
		...
		t.Logf("UserAuthenticator without token passed want %v got %v", 401,status)
	}
	...
	if status, resp := testRequest(t, ts, "GET", "/", h, nil); status != 401 || resp != "Unauthorized\n" {
		...
		t.Logf("UserAuthenticator with wrong secret passed want %v got %v", 401, status)
	}
	...
	if status, resp := testRequest(t, ts, "GET", "/", h, nil); status != 401 || resp != "Unauthorized\n" {
		...
		t.Logf("UserAuthenticator with wrong token passed want %v got %v",401, status)
	}
	...
	if status, resp := testRequest(t, ts, "GET", "/", h, nil); status != 401 || resp != "Unauthorized\n" {
		...
		t.Logf("UserAuthenticator with wrong token secret and wrong alg passed want %v got %v",401,status)
	}
	...
	if status, resp := testRequest(t, ts, "GET", "/", h, nil); status != 401 || resp != "Unauthorized\n" {
		...
		t.Logf("UserAuthenticator with correct token secret but wrong alg passed want %v got %v",401, status)
	}
	...
	if status, resp := testRequest(t, ts, "GET", "/", newAuthHeader(adminClaims), nil); status != 401 || resp != "Unauthorized\n" {
		...
		t.Logf("UserAuthenticator with wrong claim passed want %v got %v",401, status)
	}
	...
	if status, resp := testRequest(t, ts, "GET", "/", newAuthHeader(userClaims), nil); status != 200 || resp != "welcome" {
		...
		t.Logf("UserAuthenticator with correct claim passed want %v got %v", 200, status)
	}
}
